...
//让文章分类和我们的联系在一起（回显文章分类）
import {
  articleCategoryListService,
  articleListService,
  articleaddService,
} from '@/api/article.js'
const articleCategoryLis = async () => {
...
const articleList = async () => {
  ...
  let lists = await articleListService(params)
  //渲染视图
  total.value = lists.data.total
  articles.value = lists.data.items
  //处理数据给模型扩展一个分类名称
...
//添加文章
const addarticle = async (val) => {
  articleModel.value.state = val
  let addarticle = await articleaddService(articleModel.value)
    ...
    <el-form inline>
      <el-form-item label="文章分类：">
        <el-select placeholder="请选择"
                   v-model="categoryId">
          <el-option v-for="c in categorys"
                     :key="c.id"
      ...
      <el-form-item>
        <el-button type="primary"
                   @click="articleList()">搜索</el-button>
